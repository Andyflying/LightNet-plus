...
class ModulatedDeformConvTM(ModulatedDeformConv):
    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding,
                 dilation=1, groups=1, deformable_groups=1, im2col_step=64, bias=True, lr_mult=0.1):
        super(ModulatedDeformConvTM, self).__init__(in_channels, out_channels,
                                                      kernel_size, stride, padding, dilation, groups,
                                                      deformable_groups, im2col_step, bias)
        ...
        self.conv_offset_mask = nn.Conv2d(self.in_channels,
                                          ...
                                          padding=self.padding,
                                          bias=True)
        self.weight_t = nn.Parameter(torch.zeros([3, ], dtype=torch.float32))
        self.conv_offset_mask.lr_mult = lr_mult
        self.init_offset()
    ...
    def forward(self, input, t):
        ...
        offset = torch.cat((o1, o2), dim=1)
        mask_val = torch.sigmoid(self.weight_t[0] * (t ** 2) + self.weight_t[1] * t + self.weight_t[2])
